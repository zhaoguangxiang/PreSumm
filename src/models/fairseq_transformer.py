...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        self.layer_norm_self_attn = LayerNorm(self.embed_dim)
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False):
        ...
        # use layerNorm rather than FusedLayerNorm for exporting.char_inputs can be used to determine this. remove this once we update apex with the fix
        export = getattr(args, 'char_inputs', False)
        self.layer_norm_self_attn = LayerNorm(self.embed_dim, export=export)
        self.layer_norm_context_attn = LayerNorm(self.embed_dim, export=export)
        self.layer_norm_ffn = LayerNorm(self.embed_dim, export=export)
    ...
    def forward(
        ...
        if self.encoder_attn is not None:
            residual = x
            x = self.maybe_layer_norm(self.layer_norm_context_attn, x, before=True)
            q2 = self.fc3(x)
            ...
            x = F.dropout(x, p=self.dropout, training=self.training)
            x = residual + x
            x = self.maybe_layer_norm(self.layer_norm_context_attn, x, after=True)
...
class MultiheadAttentionEncoder(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, args, dropout=0., bias=True):
        ...
        self.fc1 = Linear(embed_dim, 3*embed_dim)
        self.fc2 = Linear(embed_dim, embed_dim)
...
class MultiheadAttentionDecself(nn.Module):
    ...
    def __init__(self, embed_dim, num_heads, args, kdim=None, vdim=None, dropout=0.,):
        super().__init__()
        self.embed_dim = embed_dim
